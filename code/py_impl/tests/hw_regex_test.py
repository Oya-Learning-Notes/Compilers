...
final_dfa = final_fa.to_dfa().__deepcopy__()
dfa_graph = FADiGraph(name='Determined', fa=final_dfa).render(format='png')

min_dfa = final_dfa.minimize()

graph = FADiGraph(name='Minimized Regex', fa=min_dfa)
